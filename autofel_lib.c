...
{    
    ...
    fprintf(stderr,"Sending request for version...\n\n");
	
	aw_send_fel_request(dev, AW_FEL_VERSION, 0, 0);
	
	fprintf(stderr,"Reading reply...\n\n");
	aw_usb_read(dev, buf, sizeof(*buf));
	
	fprintf(stderr,"Trying to get status...\n\n");
...
void SendBurnHandshake(feldev_handle *dev, int op)
        {
            //EnsureBurnMode();
			...
			//usb_bulk_send(dev->usb->handle, dev->usb->endpoint_out,&cbw, sizeof(cbw), true);
			...
			//rc=libusb_bulk_transfer(dev->usb->handle, dev->usb->endpoint_out, (unsigned char*)&cbw, 31, sizeof(cbw), 1000);
			rc=libusb_bulk_transfer(dev->usb->handle, dev->usb->endpoint_out, (unsigned char*)&cbw, 31, &sent, 1000);
            fprintf(stderr, "finished: status:%d...%d\n\n\n", rc,sent);
			   ...
			   uint16_t vendor_id, uint16_t product_id)
{
	...
	if (busnum < 0 || devnum < 0) {
		 ...
		 */
		fprintf(stderr,"Opening usb device: %d...\n\n", vendor_id);
	...
	//fprintf(stderr,"trying to claim device...\n\n");
	feldev_claim(result); /* claim interface, detect USB endpoints */
    //fprintf(stderr,"got past that...\n\n");
	
	//fprintf(stderr,"trying to send burn handshake enter FEL..\n\n");
	//SendBurnHandshake(result,2);
	//SendBurnHandshake(result,0);
	SendBurnHandshake(result,2);
	//fprintf(stderr,"Sent that... now getting reply...\n\n");
	
	//char buf[1000];
	//aw_usb_read(result, buf, sizeof(buf));
	
	//fprintf(stderr, "the reply to that was: %s", buf);
	//aw_usb_read(result, buf, sizeof(*buf));
	
	//fprintf(stderr,"trying to write value...\n\n");
	//fel_writel(result, 0, 1);
	//fel_writel_n(result, 2, valueToPass, 1);
	fprintf(stderr,"Astro City Mini should be in FEL mode now.\n\n");
	sleep(4);
	exit(0);
	/* retrieve BROM version and SoC information */
				
				
	//fprintf(stderr,"trying to get fel version...\n\n");
	//aw_fel_get_version(result, &result->soc_version);
	//fprintf(stderr,"got past that...\n\n");
	
	//fprintf(stderr,"trying to get soc name from id...\n\n");
	...
	//fprintf(stderr,"going to return the result...\n\n");
	return result;
}
